...
/// TR: Ücretsiz geocoding ve yer arama için Nominatim (OpenStreetMap) servisi
/// 
/// This replaces Google Places API with free OpenStreetMap data
/// Bu, Google Places API'yi ücretsiz OpenStreetMap verisiyle değiştirir
/// 
/// Rate Limits / Hız Limitleri:
/// - 1 request per second / saniyede 1 istek
/// - User-Agent header required / User-Agent header'ı gerekli
/// 
/// Cost: FREE ✅ / Maliyet: ÜCRETSİZ ✅
class NominatimService {
  ...
  /// TR: Hız limitini uygula (saniyede 1 istek)
  ...
  /// TR: Bir konumun yakınındaki yerleri (restoranları) ara
  /// 
  /// Replaces: Google Places Nearby Search
  /// Değiştirir: Google Places Nearby Search
  Future<List<NominatimPlace>> searchNearby({
    ...
    try {
      ...
      // Calculate bounding box / Sınırlayıcı kutu hesapla
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('✅ Nominatim returned ${results.length} results');

        final places = results
            .map((r) => NominatimPlace.fromJson(r as Map<String, dynamic>))
            .toList();

        // Filter by distance / Mesafeye göre filtrele
        ...
        // Sort by distance / Mesafeye göre sırala
  ...
  /// Değiştirir: Google Places Text Search
  Future<List<NominatimPlace>> searchByText({
    ...
    try {
      ...
      // Add location bias if provided / Konum önyargısı ekle
  ...
  /// Değiştirir: Google Places Autocomplete
  Future<List<String>> autocomplete({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Extract unique display names / Benzersiz görünen adları çıkar
  ...
  /// TR: İki koordinat arasındaki mesafeyi hesapla (Haversine formülü)
  ...
  /// Dispose resources / Kaynakları temizle
  void dispose() {
    _client.close();
  }
}
